...
public class KakaoService {
	...
	public String getAccessToken(HttpSession session, String code, String state) {
		...
		if(sessionValue.equals(state)) { //로그인할 때 만든 state와 성공 후 응답된 state가 일치하는가
			
			//요청 헤더 설정
			HttpHeaders headers =new HttpHeaders();
			//카카오 서버쪽에서 설정해달라고 한 content-type 설정하기
			...
			//카카오 서버로 rest(비동기) 방식의 post 통신을 보내줄 객체를 생성
	...
	public MKakaoUserVO getUserProfile(String accessToken) {
		...
		//요청 보내기
		RestTemplate template = new RestTemplate();
		ResponseEntity<MKakaoUserVO> responseEntity = template.exchange(
			requestUri,
			HttpMethod.GET,
		...
		//응답 바디 읽기.
		MKakaoUserVO responseData = responseEntity.getBody();
